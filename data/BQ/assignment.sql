...
CREATE TABLE IF NOT EXISTS `esoteric-virtue-477914-t2.gold_dataset.financial_metrics` AS
SELECT 
...
CREATE TABLE IF NOT EXISTS `esoteric-virtue-477914-t2.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `esoteric-virtue-477914-t2.silver_dataset.claims` c
